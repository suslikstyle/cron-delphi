...
**Delphi Cron Scheduler** — это класс на Delphi, реализующий планировщик заданий, работающий по принципу Unix-cron, а также поддерживающий отложенные разовые задачи. 
Позволяет выполнять задачи по расписанию: каждую минуту, каждые N минут, в определённое время суток, по дням недели и т.д.

Поддерживает стандартный синтаксис cron-масок (`*`, `*/N`, `A-B`, `A,B,C`, `?`), синтаксис разовых задач (`+5m`, `+2h`) и многопоточную работу через `TThread`.

---

## 🚀 Возможности

- **Поддержка cron-масок формата:** `* * * * *` каждую минуту  
...
- **Разовые отложенные задачи (One-time tasks):** Автоматически очищаются из памяти после выполнения.
...
begin
	...
	// Добавление разовых отложенных задач
  	FCron.AddTask('Уведомление через 30 секунд', '+30s');
  	FCron.AddTask('Очистка кеша через 2 часа', '+2h');
	
	Readln; // не даем завершиться

	// Завершение работы
	FCron.Free; // Автоматически вызовет Stop/Terminate
end.
```
